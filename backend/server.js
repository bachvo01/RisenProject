...
app.get('/api/top/sellers', async(req, res) => {
    try {
        const query = await pool.query("WITH topSellers AS ( SELECT COUNT(productid), productid FROM rsn_order NATURAL JOIN rsn_order_items GROUP BY productid) SELECT productid, title, price, filecover1, count FROM topSellers NATURAL JOIN rsn_product ORDER BY count DESC")
...
app.post('/refresh', (req,res) => {
    try {
        ...
        const getRefresh = async (token) => {
            try {
                const query = await pool.query("SELECT EXISTS (SELECT 1 FROM rsn_user WHERE refreshtk = $1)",
                [token])
                
                const userQuery = await pool.query("SELECT userid, email FROM rsn_user WHERE refreshtk = $1",
                [token])
                ...
                if(query.rows[0].exists === false){
                    ...
                    res.json({isAuthenticated : true, accessUserToken: newAccessToken, refreshUserToken : newRefreshToken})
                }
                else{
                    const newAccessToken = generateAccessToken(user)
                    console.log("New: ", newAccessToken)
                    res.json({isAuthenticated : true, accessUserToken: newAccessToken})
                }  
...
const addRefreshTK = async (token, user) => {
    const query = await pool.query("UPDATE rsn_user SET refreshtk = $1 WHERE userid = $2",
    [token, user.userid])
}
    ...
    try {
        ...
        else
        {
            if(await bcrypt.compare(password, registeredUser.password))
            {
                ...
                res.json({isAuthenticated : true, accessUserToken: accessToken, refreshUserToken : refreshToken}) 
    ...
    try {
        ...
        if(authHeader) {
            ...
            jwt.verify(token, JWT_SECRET_KEY, async (err, userDetails) => {
                ...
                .then(async(query) => {
                    const userid = query.rows[0].userid
                    const orderQuery = await pool.query("SELECT orderid, totalprice, ordered_at FROM rsn_user NATURAL JOIN rsn_order NATURAL JOIN rsn_order_items WHERE userid = $1 ORDER BY ordered_at DESC", [userid])
                    const orderDetails = orderQuery.rows[0]
                    const itemsQuery = await pool.query("SELECT productid, code, itemid, title, price, filecover1 FROM rsn_order NATURAL JOIN rsn_order_items NATURAL JOIN rsn_product WHERE orderid = $1", [orderDetails.orderid])
